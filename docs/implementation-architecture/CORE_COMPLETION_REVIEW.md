...
| **Covers** | Implementations 001–006 (core) · **updated for 007 (Purpose `athlete`), 008 (projection freshness), 009 (AthleteDecision feedback), 010 (persistence ports + in-memory repositories), 011 (event/outcome records + traceability envelope), 012 (check-only reprojection harness), 013 (manual input adapter), 014 (deterministic rendering boundary), 015 (rendered-message record/review persistence), 016 (delivery boundary)** |
| **Validation at writing** | core: `145/145` · 007: `175/175` · 008: `196/196` · 009: `212/212` · 010: `235/235` · 011: `275/275` · 012: `295/295` · 013: `317/317` · 014: `349/349` · 015: `374/374` · **post-016: `tsc --noEmit` clean · `node --test` 397/397 pass** |
| **Modules** | `observation` (intake **+ Manual Input Adapter**, Impl 013), `reasoning`, `understanding` (projection freshness), `decision-support`, **`athlete` (Purpose + AthleteDecision)** (+ `shared-kernel`) — each with **repository ports + in-memory adapters** (Impl 010) — **plus the dependency-neutral `event-recording` module** (append-only, ref-only `DomainEventRecord` log) (Impl 011) — **plus a neutral check-only `reprojection-harness`** (test-support seam, not a module) (Impl 012) — **plus the downstream `rendering` module** (deterministic fake renderer + mandatory validator (Impl 014) **+ rendered-message record/review repository (Impl 015)**) — **plus the downstream `delivery` module** (display-eligibility-gated exposure to a deterministic test sink + audit records (Impl 016)) |
...
- **What does not exist yet:** no UI, API, **production DB/persistence**, **real LLM provider** (a deterministic rendering boundary exists — Impl 014 — but no provider/prompts), **real delivery channel/provider** (a delivery boundary with a deterministic test sink exists — Impl 016 — but no email/SMS/push/web channel), event bus, notification layer, scheduler/retry layer, Garmin/FIT adapter, or production orchestration service. The `athlete` module now holds **Purpose (Impl 007) + AthleteDecision (Impl 009)** — but **full Athlete** (state/capacity/constraints/path-memory), a **full `DecisionOutcome` object**, a **pattern engine**, and any **compliance/outcome-based validation** are still absent. These are **intentional absences** (see §6), not gaps left by mistake.
...
| Real LLM provider / prompt templates / external delivery | **Intentional** | The rendering boundary is proven with a **deterministic fake renderer**; a real provider, production prompts, and delivery remain future, behind the same validator contract. |
...
1. **Spec 017 — Provider Adapter Boundary *or* Delivery Event Surface** *(recommended next — choose explicitly)*
   - *Why:* with rendering generated/recorded/reviewed and delivery exposing display-eligible messages to a test sink, the remaining edges are (a) wiring a **real LLM provider behind the same mandatory rendering validator**, or (b) recording **delivery occurrences as ref-only events** (a delivery event surface) — and, further out, a **real channel** behind the `DeliverySink`. Each is a genuine "real outside world" boundary and should be opened deliberately, one at a time.
   - *Must not:* let a provider skip the mandatory validator; let a delivery event become a command/retry trigger; let a generated/delivered artifact become source truth; introduce UI/API/provider/channel without a strict boundary contract.
   - *Depends on:* `rendering` (validator/record/review, done) + `delivery` (eligibility-gated exposure + audit, done); a deliberate choice of which edge to open first.
...
[DECISION] **Recommended next mission: Spec 017 — Provider Adapter Boundary *or* Delivery Event Surface (choose explicitly).** With *data in* (Impl 013), *output out* (Impl 014), an **auditable record/review** of rendered messages (Impl 015), and an **eligibility-gated delivery boundary** to a deterministic test sink (Impl 016) all proven in-memory, the remaining edges touch the **real outside world**: (a) wiring a **real LLM provider behind the same mandatory rendering validator**, (b) recording **delivery occurrences as ref-only events**, and, further out, attaching a **real channel** behind the `DeliverySink`. Each is a deliberate boundary to open one at a time — a provider must never skip the validator, a delivery event must never become a command/retry trigger, delivery must never bypass display eligibility, and no generated/delivered artifact may become source truth. The discipline that produced restraint by construction must hold precisely when a demo-able surface becomes tempting.
...
Anything involving **a real provider/channel/surface, transport, durable storage, and production recomputation**: a **real delivery channel/provider** and a **UI / API** (an eligibility-gated delivery boundary to a deterministic *test sink* exists, but no real channel), a **real LLM provider** + production prompts (a deterministic rendering boundary **and** an auditable record/review exist, but no provider), **external (FIT/wearable) ingestion**, **rendered-output / delivery event records**, a **scheduler/retry layer**, a **production** persistence backend / event store, a production orchestration entrypoint, the **full** `athlete` model (state/capacity/constraints/path-memory), risk from real data, reasoning that is **purpose-version-aware** with a reinterpretation engine, a **generic projection engine** (`ImpactAssessment`, event-driven/persisted refresh), a **full `DecisionOutcome` / decision-pattern engine**, and a **production reprojection service / scheduler / projection repository**. Purpose **is** a real, versioned upstream source (Impl 007), **projection freshness is explicit** (Impl 008), the **decision feedback loop is built** (Impl 009), **aggregates now round-trip through validated ports + in-memory repositories** (Impl 010), **occurrences are now recorded as an append-only, ref-only event/outcome log** (Impl 011), **derived state can now be recomputed and verified by a neutral check-only harness** (Impl 012), **real manual data now enters as faithful `ObservationSet`s through an observation-owned adapter** (Impl 013), **domain-approved terminal outputs now render to human-facing text through a deterministic, validated boundary** (Impl 014), **rendered messages are now conserved and reviewed as auditable presentation artifacts** (Impl 015), and **display-eligible messages are now exposed to a deterministic test sink + audit records through a downstream delivery boundary** (Impl 016) — all with **no technology chosen**. The linear path is proven, **three domain loops are exercised** (purpose-change staleness, projection freshness/refresh, decision feedback — all no-rewrite), **persistence is proven safe in-memory**, **the event surface is realized as inert, ref-only records**, **reprojection is proven to recompute-and-verify without mutating/executing/asserting truth**, **the first real ingress records source material without interpreting it**, **the first real egress expresses the decision without becoming authority**, **the output-out cycle is now auditable without the record becoming authority**, and **the exposure edge delivers only display-eligible messages without delivery becoming evidence, authority, or a real channel**; the remaining work (a real provider/channel surface, a delivery event surface, a production transport/storage backend, deep purpose reinterpretation, and learning *over time*) is specified or scoped but not yet built.

[ASSUMPTION] **What is the most dangerous next shortcut?**
**Wiring a real LLM provider, a real delivery channel, or a UI/API surface that bypasses the rendering validator or the display-eligibility gate (or treats a rendered/delivered message as truth).** The rendering *boundary* is proven safe with a deterministic fake renderer + a mandatory validator (Impl 014), and the delivery *boundary* is proven safe with a deterministic test sink behind `displayEligibilityOf` (Impl 016); the danger shifts to the moment a real provider, a real channel, or a demo-able surface is attached — letting generated text skip the validator, letting delivery skip eligibility, deriving voice from convenience instead of the gates, or re-ingesting a rendered/delivered artifact as source truth would reintroduce the exact collapses the core was built to prevent, and it would not look wrong from the inside. The discipline must hold precisely when a real mouth and a real channel become tempting: **a real provider goes behind the same validator; a real channel goes behind the same eligibility gate; a rendered or delivered message is never source truth; delivery success is never evidence; the surface never selects voice.**
...
- `npm test` (`node --test "src/**/*.test.ts"`) — **397 / 397 pass**, 0 fail (was 145 at the 006 baseline; +30 for Impl 007, +21 for Impl 008, +16 for Impl 009, +23 for Impl 010, +40 for Impl 011, +20 for Impl 012, +22 for Impl 013, +32 for Impl 014, +25 for Impl 015, +23 for Impl 016), including all module/negative/boundary tests, the Implementation 006 end-to-end Reflection proof, and the Impl 007/008/009/010/011/012/013/014/015/016 purpose + projection-freshness + decision-feedback + persistence-round-trip + event-record + reprojection-harness + manual-input + rendering + rendered-message-record/review + delivery tests.
