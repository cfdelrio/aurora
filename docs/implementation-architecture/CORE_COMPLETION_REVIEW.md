...
| **Covers** | Implementations 001–006 (core) · **updated for 007 (Purpose `athlete`), 008 (projection freshness), 009 (AthleteDecision feedback), 010 (persistence ports + in-memory repositories), 011 (event/outcome records + traceability envelope), 012 (check-only reprojection harness), 013 (manual input adapter), 014 (deterministic rendering boundary)** |
| **Validation at writing** | core: `145/145` · 007: `175/175` · 008: `196/196` · 009: `212/212` · 010: `235/235` · 011: `275/275` · 012: `295/295` · 013: `317/317` · **post-014: `tsc --noEmit` clean · `node --test` 349/349 pass** |
| **Modules** | `observation` (intake **+ Manual Input Adapter**, Impl 013), `reasoning`, `understanding` (projection freshness), `decision-support`, **`athlete` (Purpose + AthleteDecision)** (+ `shared-kernel`) — each with **repository ports + in-memory adapters** (Impl 010) — **plus the dependency-neutral `event-recording` module** (append-only, ref-only `DomainEventRecord` log) (Impl 011) — **plus a neutral check-only `reprojection-harness`** (test-support seam, not a module) (Impl 012) — **plus the downstream `rendering` module** (deterministic fake renderer + mandatory validator) (Impl 014) |
...
| 42 | A real LLM provider / `rendering` coupling / domain mutation by rendering | `rendering` imports only `shared-kernel` + read-only `decision-support` *types*; no domain module imports it; structural guard asserts no `src/{llm,api,ui,infrastructure}` / `src/modules/{llm,openai,provider}` and no provider/network token; the fake renderer is deterministic; rendering mutates/persists/emits nothing (Impl 014 structural + neutral tests) |
...
| Rendered-message persistence / rendered-output event records | **Intentional** | A `RenderedMessage` is returned as an output object only — no repository, no `RenderedMessageCreated` event (ref-only, if ever). |
...
1. **Spec 015 — Rendered-Message Review / Persistence (or another explicitly chosen slice)** *(recommended next)*
   - *Why:* with rendering proven safe in-memory, the next step is *what happens to a rendered message* — an append-only, ref-only record of "rendered" and/or a review/approval workflow — **before** a real provider or surface is wired.
   - *Must not:* let a rendered message become source truth; write an event-as-command; persist generated text as domain authority; introduce a provider/UI/API by the back door.
   - *Depends on:* `rendering` (done); `event-recording`/persistence ports (done).
...
[DECISION] **Recommended next mission: Spec 015 — Rendered-Message Review / Persistence (or another explicitly chosen slice).** With both boundaries now proven — *data in* (Impl 013) and *output out* (Impl 014) — and the full pipeline round-trippable/recordable/recompute-verified (Impl 010–012), the next step is what *happens to* a rendered message: an append-only, ref-only "rendered" record and/or a review/approval workflow, kept downstream and never source truth — **before** a real LLM provider or a UI/API surface is wired (each behind the same validator/boundary contracts). The discipline that produced restraint by construction must hold precisely when a demo-able surface becomes tempting.
...
- `npm test` (`node --test "src/**/*.test.ts"`) — **349 / 349 pass**, 0 fail (was 145 at the 006 baseline; +30 for Impl 007, +21 for Impl 008, +16 for Impl 009, +23 for Impl 010, +40 for Impl 011, +20 for Impl 012, +22 for Impl 013, +32 for Impl 014), including all module/negative/boundary tests, the Implementation 006 end-to-end Reflection proof, and the Impl 007/008/009/010/011/012/013/014 purpose + projection-freshness + decision-feedback + persistence-round-trip + event-record + reprojection-harness + manual-input + rendering tests.
