...
| **Covers** | Implementations 001–006 (core) · **updated for 007 (Purpose `athlete`), 008 (projection freshness), 009 (AthleteDecision feedback), 010 (persistence ports + in-memory repositories), 011 (event/outcome records + traceability envelope), 012 (check-only reprojection harness), 013 (manual input adapter), 014 (deterministic rendering boundary), 015 (rendered-message record/review persistence)** |
| **Validation at writing** | core: `145/145` · 007: `175/175` · 008: `196/196` · 009: `212/212` · 010: `235/235` · 011: `275/275` · 012: `295/295` · 013: `317/317` · 014: `349/349` · **post-015: `tsc --noEmit` clean · `node --test` 374/374 pass** |
| **Modules** | `observation` (intake **+ Manual Input Adapter**, Impl 013), `reasoning`, `understanding` (projection freshness), `decision-support`, **`athlete` (Purpose + AthleteDecision)** (+ `shared-kernel`) — each with **repository ports + in-memory adapters** (Impl 010) — **plus the dependency-neutral `event-recording` module** (append-only, ref-only `DomainEventRecord` log) (Impl 011) — **plus a neutral check-only `reprojection-harness`** (test-support seam, not a module) (Impl 012) — **plus the downstream `rendering` module** (deterministic fake renderer + mandatory validator (Impl 014) **+ rendered-message record/review repository (Impl 015)**) |
...
| Rendered-output **event records** | **Intentional** | A `RenderedMessageRecord` is **not** an event record; `rendering` imports no `event-recording`; the catalog is not expanded (ref-only `RenderedMessageRecorded`/`RenderReviewed`, if ever). |
| **Delivery / UI / API** for reviewed messages | **Intentional** | Display eligibility is **derived, not delivery**; how an approved message reaches a human (UI/API/notification) is future, downstream of the record. |
| Rendered-message **production persistence** | **Intentional** | The record repository is **in-memory** (port + adapter); no production DB/schema/ORM/migration. |
...
1. **Spec 016 — Delivery Boundary *or* Provider Adapter Boundary** *(recommended next — choose explicitly)*
   - *Why:* with rendering generated, recorded, and reviewed safely, the remaining edges are (a) **delivering** a display-eligible message to a human surface, or (b) wiring a **real LLM provider behind the same validator**. Either is a genuine "real outside world" boundary and should be opened deliberately, one at a time.
   - *Must not:* let delivery bypass display eligibility/review; let a provider skip the mandatory validator; let a delivered/generated artifact become source truth; introduce UI/API/provider without a strict boundary contract.
   - *Depends on:* `rendering` (record/review/validator, done); a deliberate choice of which edge to open first.
...
[DECISION] **Recommended next mission: Spec 016 — Delivery Boundary *or* Provider Adapter Boundary (choose explicitly).** With *data in* (Impl 013), *output out* (Impl 014), and an **auditable record/review** of rendered messages (Impl 015) all proven in-memory, the remaining edges touch the **real outside world**: (a) **delivering** a display-eligible, reviewed message to a human surface, or (b) wiring a **real LLM provider behind the same mandatory validator**. Each is a deliberate boundary to open one at a time — delivery must never bypass display eligibility/review, and a provider must never skip the validator or let a generated/delivered artifact become source truth. The discipline that produced restraint by construction must hold precisely when a demo-able surface becomes tempting.
...
Anything involving **a real provider/surface, transport, durable storage, and production recomputation**: a **delivery / UI / API** surface, a **real LLM provider** + production prompts (a deterministic rendering boundary **and** an auditable record/review exist, but no provider/delivery), **external (FIT/wearable) ingestion**, **rendered-output event records**, a **production** persistence backend / event store, a production orchestration entrypoint, the **full** `athlete` model (state/capacity/constraints/path-memory), risk from real data, reasoning that is **purpose-version-aware** with a reinterpretation engine, a **generic projection engine** (`ImpactAssessment`, event-driven/persisted refresh), a **full `DecisionOutcome` / decision-pattern engine**, and a **production reprojection service / scheduler / projection repository**. Purpose **is** a real, versioned upstream source (Impl 007), **projection freshness is explicit** (Impl 008), the **decision feedback loop is built** (Impl 009), **aggregates now round-trip through validated ports + in-memory repositories** (Impl 010), **occurrences are now recorded as an append-only, ref-only event/outcome log** (Impl 011), **derived state can now be recomputed and verified by a neutral check-only harness** (Impl 012), **real manual data now enters as faithful `ObservationSet`s through an observation-owned adapter** (Impl 013), **domain-approved terminal outputs now render to human-facing text through a deterministic, validated boundary** (Impl 014), and **rendered messages are now conserved and reviewed as auditable presentation artifacts** (Impl 015) — all with **no technology chosen**. The linear path is proven, **three domain loops are exercised** (purpose-change staleness, projection freshness/refresh, decision feedback — all no-rewrite), **persistence is proven safe in-memory**, **the event surface is realized as inert, ref-only records**, **reprojection is proven to recompute-and-verify without mutating/executing/asserting truth**, **the first real ingress records source material without interpreting it**, **the first real egress expresses the decision without becoming authority**, and **the output-out cycle is now auditable without the record becoming authority**; the remaining work (a delivery/provider surface, a production transport/storage backend, deep purpose reinterpretation, and learning *over time*) is specified or scoped but not yet built.

[ASSUMPTION] **What is the most dangerous next shortcut?**
**Wiring a real LLM provider or a UI/API surface that bypasses the rendering validator (or treats a rendered message as truth).** The rendering *boundary* is now proven safe with a deterministic fake renderer + a mandatory validator (Impl 014); the danger shifts to the moment a real provider or a demo-able surface is attached — letting generated text skip the validator, derive voice from convenience instead of the gates, or be re-ingested as source truth would reintroduce the exact collapses the core was built to prevent, and it would not look wrong from the inside. The discipline must hold precisely when a real mouth becomes tempting: **a real provider goes behind the same validator; a rendered message is never source truth; the surface never selects voice.**
...
- `npm test` (`node --test "src/**/*.test.ts"`) — **374 / 374 pass**, 0 fail (was 145 at the 006 baseline; +30 for Impl 007, +21 for Impl 008, +16 for Impl 009, +23 for Impl 010, +40 for Impl 011, +20 for Impl 012, +22 for Impl 013, +32 for Impl 014, +25 for Impl 015), including all module/negative/boundary tests, the Implementation 006 end-to-end Reflection proof, and the Impl 007/008/009/010/011/012/013/014/015 purpose + projection-freshness + decision-feedback + persistence-round-trip + event-record + reprojection-harness + manual-input + rendering + rendered-message-record/review tests.
